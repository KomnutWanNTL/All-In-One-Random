...
function displayRandomData() {
    ...
    // Create a shuffled copy of the generators
    const shuffledGenerators = [...dataGenerators].sort(() => 0.5 - Math.random());

    // Generate one data set from each generator
    let generatedData = shuffledGenerators.map(generator => generator()).filter(item => item !== null);

    // Ensure Thai ID is always first if it exists
    const idCardIndex = generatedData.findIndex(item => item.type === 'thai_id');
    if (idCardIndex > 0) {
        const idCardItem = generatedData.splice(idCardIndex, 1)[0];
        generatedData.unshift(idCardItem);
    }
